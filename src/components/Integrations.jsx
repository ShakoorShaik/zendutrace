  ...
  return (
    <div
      ...
      style={{
        ...
        padding: '20px 22px',
        background: '#FFFFFF',
        border: hovered ? '1px solid rgba(194,65,12,0.3)' : '1px solid rgba(13,16,20,0.08)',
        boxShadow: hovered ? '0 18px 44px -28px rgba(13,16,20,0.35)' : '0 1px 2px rgba(13,16,20,0.03)',
        transform: hovered ? 'translateY(-3px)' : 'none',
        transition: 'border-color .2s,transform .2s,box-shadow .2s',
      }}
      {...hoverProps}
    >
      <div className="integration-mark" style={{ flexShrink: 0, width: 46, height: 46, borderRadius: 12, background: ig.chip, color: ig.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Helvetica Neue',Helvetica,Arial,sans-serif", fontWeight: 700, fontSize: 15, letterSpacing: '0.01em' }}>
        {ig.mono}
      </div>
      <div style={{ minWidth: 0 }}>
        <div style={{ fontFamily: "'Helvetica Neue',Helvetica,Arial,sans-serif", fontWeight: 600, fontSize: 15.5, color: '#0F1114' }}>{ig.name}</div>
        <div style={{ marginTop: 2, fontSize: 12.5, color: '#878E96' }}>{ig.cat}</div>
      </div>
    </div>
  );
}
    ...
    <section id="integrations" className="integrations-section section-shell" style={{ maxWidth: 1240, margin: '0 auto', padding: '80px 32px' }}>
      ...
      <div className="api-grid api-panel" style={{ marginTop: 20, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0, borderRadius: 22, overflow: 'hidden', background: '#0F1114', border: '1px solid rgba(13,16,20,0.5)' }}>
        <div style={{ padding: '44px 42px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <h3 style={{ margin: 0, fontFamily: "'Helvetica Neue',Helvetica,Arial,sans-serif", fontWeight: 700, fontSize: 'clamp(22px,2.4vw,30px)', lineHeight: 1.12, letterSpacing: '-0.02em', color: '#FFFFFF' }}>
            Build anything on the open API
          </h3>
          <p style={{ marginTop: 14, fontSize: 15, lineHeight: 1.6, color: '#B9C0C8', maxWidth: '26rem' }}>
        ...
        <div style={{ position: 'relative', padding: 32, background: '#12100E', borderLeft: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center' }}>
          <pre style={{ margin: 0, width: '100%', fontFamily: "'SFMono-Regular',Menlo,Consolas,monospace", fontSize: 13, lineHeight: 1.7, color: '#B9C0C8', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
            <span style={{ color: '#FF8A2B' }}>POST</span> /v1/labels/<span style={{ color: '#7FD1B9' }}>LZ-4821</span>/subscribe
            {'\n\n'}
            {'{\n  '}
            <span style={{ color: '#5CB3F8' }}>&quot;events&quot;</span>
            {': ['}
            ...
            {'],\n  '}
            <span style={{ color: '#5CB3F8' }}>&quot;webhook&quot;</span>
            {': '}
            <span style={{ color: '#7FD1B9' }}>&quot;https://hooks.acme.com/zendu&quot;</span>
            {'\n}'}
            {'\n\n'}
            <span style={{ color: '#1E8A5B' }}>&rarr; 200 OK</span> <span style={{ color: '#5C636B' }}>&middot;</span> <span style={{ color: '#878E96' }}>streaming live</span>
          </pre>
        </div>
      </div>
    </section>
